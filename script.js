    ...
    document.addEventListener("DOMContentLoaded", loadAllItems(menu));
    document.addEventListener("DOMContentLoaded", loadCategoryButton);

    function loadAllItems(menu) {
    ...
    function loadCategoryButton() {
      ...
      buttonContainer.innerHTML = categories
        .map((category) => {
          return `<button class="btn-item" type="button" data-category="${category}">${category}</button>`;
        })
        .join("");
